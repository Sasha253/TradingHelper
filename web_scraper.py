...
class ViewPoint:
    ...
    def __init__(self, stock_id):
        ...
        def login(driver_type):

            driver_type.get("https://securities.standardbank.co.za/ost/")

            search_username = driver_type.find_element_by_id("normalUsername")
            search_username.clear()
            search_username.send_keys("sashmul055462")
            search_password = driver_type.find_element_by_id("j_password")
            search_password.clear()
            search_password.send_keys("Sasha@23")

            login_element = driver_type.find_element_by_id("submitButton")
            login_element.click()

            driver_type.implicitly_wait(5)

            driver_type.get("https://viewpoint.iress.co.za/st/")  # TODO might have to do a element load first

            driver_type.implicitly_wait(35)

        def navigate_to_stock():     # TODO sbk_stock: count = 0, mnp_stock:count = 1, etc...
        ...
        def buy_or_sell_stock(is_buy, is_sell, volume, order_type, limit_price, life_time):
